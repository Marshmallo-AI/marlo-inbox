    ...
    <div className="border border-border rounded-lg p-3 mb-2 bg-muted/50">
      ...
      {toolCall.args && Object.keys(toolCall.args).length > 0 && (
        <div className="mb-2">
          ...
          <div className="bg-background rounded px-3 py-2 text-xs font-mono border border-border overflow-auto max-h-32">
      ...
      {messageContent && !isRunning && (
        <div>
          ...
          <div className="bg-green-50 dark:bg-green-900/20 rounded px-3 py-2 text-xs border border-green-200 dark:border-green-800 overflow-auto max-h-32">
...
export function ChatMessageBubble(props: {
  ...
  if (
    !(
      ["human", "ai"].includes(props.message.type) &&
      (hasContent || shouldShowToolCalls)
    )
  ) {
    return null
  }
  ...
  return (
    ...
    >
      {props.message.type === "ai" && (
        <div className="mr-4 border bg-secondary rounded-full w-10 h-10 flex-shrink-0 flex items-center justify-center text-lg">
          {props.aiEmoji}
        </div>
      )}
